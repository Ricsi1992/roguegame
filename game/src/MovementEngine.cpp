...
{
    if ((t_position.y == 1 && t_facing == Direction::UP)
        || (t_position.y == t_maxHeight - 2 && t_facing == Direction::DOWN)
        || (t_position.x == 1 && t_facing == Direction::LEFT)
        || (t_position.x == t_maxWidth - 2 && t_facing == Direction::RIGHT))
    {
        return true;
    }
    return false;
